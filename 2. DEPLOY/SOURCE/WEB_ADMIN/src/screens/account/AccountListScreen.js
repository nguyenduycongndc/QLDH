import { Col, Input, Row, Select, Empty, notification, Checkbox } from "antd";
...
function AccountListScreen(props) {
  ...
  // const [isCheckToRemoveOrder, setIsCheckToRemoveOrder] = useState(false);
  ...
  const ModalAddEdit = () => {
    ...
    const chosenValues = {
      ...
      status: value?.status,
      isCheck: value?.checkRole === 1 ? true : false,
    };
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              ...
              if (!isSubmitLocked) {
                ...
                if (shouldAddItem) {
                  const payload = {
                    userName: values.employeeName.trim(),
                    phoneNumber: values.phoneNumber.trim(),
                    email: values.employeeMail.trim(),
                    password: values.employeePassword.trim(),
                    confirmPassword: values.confirmPassword.trim(),
                    checkRole: values.isCheck ? 1 : 0,
                  };
                ...
                <Form>
                  ...
                  <ModalBody>
                    ...
                    {shouldAddItem ? (
                      ...
                      <div className="mb-2">
                        <label className="mb-0">Số điện thoại/电话号码</label>
                        <br />
                        <a className="text-info" style={{ fontSize: 18 }}>
                          <b>{value?.phone}</b>
                        </a>
                      </div>
                    )}
